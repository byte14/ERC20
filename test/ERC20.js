...
describe("ERC20 Contract", () => {
  ...
  describe("transfer", () => {
    it("transfer tokens from owner account to bob account", async () => {
      await contract.transfer(bob.address, ethers.utils.parseEther('300'));
    ...
    const transferTx = await contract.transfer(bob.address, ethers.utils.parseEther('300'));
    ...
    it("throws when transfer to the zero address", async () => {
      const transferTx = contract.transfer(ethers.constants.AddressZero, ethers.utils.parseEther('300'));
    ...
    it("throws when transfer more token than owned", async () => {
      const transferTx = contract.transfer(bob.address, ethers.utils.parseEther('2000'));
